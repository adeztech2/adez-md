...
async function handleMessage(sock, m) {
  if (!m || !Array.isArray(m.messages) || !m.messages.length) return;
  ...
  if (!body || !String(body).startsWith(prefix)) return;

  const args = String(body).slice(prefix.length).trim().split(/ +/);
