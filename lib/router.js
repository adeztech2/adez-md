...
async function loadCommands() {
  ...
  for (const file of files) {
    try {
      ...
      const entry = normalizeModule(mod);

      if (!entry || !entry.name) {
        console.warn(`⚠️ Skipped ${path.basename(file)}: missing command export`);
        continue;
      }

      if (commands.has(entry.name)) {
        console.warn(`⚠️ Duplicate command "${entry.name}" in ${path.basename(file)} — skipped, already loaded from another file.`);
        continue;
      }
      ...
      commands.set(entry.name, {
        ...
        execute: entry.execute || entry.default || mod.execute || mod.default,
        adminOnly: !!entry.adminOnly,
        ownerOnly: !!entry.ownerOnly
      });

      console.log(`✅ Loaded: ${entry.name} [${entry.category || 'Uncategorized'}]`);
    } catch (err) {
...
async function loadObservers() {
  ...
  for (const file of files) {
    try {
      ...
      const mod = await import(fileUrl + `?update=${Date.now()}`);
      const entry = normalizeModule(mod);
      if (entry && (entry.execute || entry.default)) {
        observers.push(entry.execute || entry.default);
...
async function handleMessage(sock, m) {
  ...
  // Allow commands typed in the bot's own "Message yourself" chat (self-chat),
  // since fromMe is always true there. Everywhere else, still ignore fromMe
  // messages to avoid the bot responding to its own outgoing replies.
  ...
  const isOwner = sender === ownerNumber || isSelfChat;
