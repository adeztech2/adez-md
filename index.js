...
const SESSION_WRITE_INTERVAL = parseInt(process.env.SESSION_WRITE_INTERVAL) || 120000;
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (errorMessage && errorMessage.includes('conflict')) {
                console.log('STREAM ERRORED CONFLICT DETECTED!');
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                ...
                console.log('Bad session detected. Clearing and restarting...');
                fs.emptyDirSync(sessionDir);
                await supabaseRequest(`bu_sessions?id=eq.${SESSION_NAME}`, {
                    method: 'DELETE'
                });
                ...
                console.log('Method Not Allowed (405). Waiting 30 seconds...');
        ...
        if (type === 'notify' && global.autoStatusView) {
            for (const msg of messages) {
                try {
                    ...
                    if (isStatus) {
                        console.log('Auto viewing status update...');
    ...
    socket.on('request-pair-code', async (phoneNumber) => {
        ...
        try {
            ...
            if (!targetNumber || targetNumber.length < 10) {
                socket.emit('error', 'Please provide a valid phone number with country code');
                return;
            }
            ...
            setTimeout(async () => {
                try {
                    const pairingCode = await pairSock.requestPairingCode(targetNumber);
                    ...
                    socket.emit('error', 'Failed to generate pair code. Try again.');
                }
            }, 3000);
